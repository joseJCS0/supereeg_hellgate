...
def electrode_search(fname, threshold=10):
    ...
    fname = os.path.join(config['og_bodir'],'.bo') #(OG)os.path.join(config['og_bodir'], fname.split('_' + freq)[0] + '.bo')
...
brain_data = glob.glob(os.path.join(data_dir,'.bo')) #(OG)glob.glob(os.path.join(data_dir, '*' + freq + '.bo'))
...
results = os.path.join(results_dir,'locs.npz') #(OG)results = os.path.join(results_dir, freq + '_locs.npz')
